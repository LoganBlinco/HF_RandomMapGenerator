...
namespace eLF_RandomMaps
{
	...
	public class SpawnCreator : ScriptableObject
	{
		...
		public HeightMap_ModifierData HeightMapModifier;

		public void PlaceMe(TerrainData terrainData, GameObject parant, int mapSize, int mapResolution)
		{
			HandleHeightMapModifier(terrainData, mapResolution, mapSize);
		...
		private void HandleHeightMapModifier(TerrainData terrainData, int mapResolution, int mapSize)
		{
			if (HeightMapModifier == null || HeightMapModifier.featureRadius <= 0 || HeightMapModifier.HeightMapMask == null) { return; }
			...
			hMap = HeightMap_BuildingSmoothing.Execute(HeightMapModifier,hMap,spawnLocation,Generator.TERRAIN_HEIGHT_LIMIT);
