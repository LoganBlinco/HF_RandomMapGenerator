...
namespace eLF_RandomMaps
{
	...
	public class Generator : MonoBehaviour
	{
		...
		//details
		public int detailMaxDensity = 7;
		public int detailMinDensity = 4;
		...
		public void Start()
		{
			Debug.Log("MYDEBUGGER: Beggining generation");
			Run_RandomSeed();
		}
		...
		public void Execute()
		{
			...
			Stopwatch stopWatchHeights = new Stopwatch();
			stopWatchHeights.Start();
			...
			Debug.Log("MYDEBUGGER: settings heightmap resolutinL: " + mapResolution);
			...
			stopWatchHeights.Stop();

			TimeSpan ts = stopWatchHeights.Elapsed;

			// Format and display the TimeSpan value.
			string elapsedTime = String.Format("{0:00}:{1:00}:{2:00}.{3:00}",
				ts.Hours, ts.Minutes, ts.Seconds,
				ts.Milliseconds / 10);
			Debug.LogWarning("RunTime heights" + elapsedTime);


			Stopwatch stopWatchProps = new Stopwatch();
			stopWatchProps.Start();
			...
			StructureSpawner.Instance.RunBorderCreator();
			...
			ts = stopWatchProps.Elapsed;

			// Format and display the TimeSpan value.
			elapsedTime = String.Format("{0:00}:{1:00}:{2:00}.{3:00}",
				ts.Hours, ts.Minutes, ts.Seconds,
				ts.Milliseconds / 10);
			Debug.LogWarning("RunTimeProps " + elapsedTime);


			Stopwatch stopWatchDetails = new Stopwatch();
			...
			ts = stopWatchDetails.Elapsed;

			// Format and display the TimeSpan value.
			elapsedTime = String.Format("{0:00}:{1:00}:{2:00}.{3:00}",
				ts.Hours, ts.Minutes, ts.Seconds,
				ts.Milliseconds / 10);
			Debug.LogWarning("RunTimeDetails " + elapsedTime);
			...
			elapsedTime = String.Format("{0:00}:{1:00}:{2:00}.{3:00}",
				ts.Hours, ts.Minutes, ts.Seconds,
				ts.Milliseconds / 10);
			Debug.Log("Total RunTime " + elapsedTime);
			...
			for (int detailIndex = 0; detailIndex < numDetails; detailIndex++)
			{
				...
				for (int y = 0; y < terrainData.detailHeight; y++)
				{
					for (int x = 0; x < terrainData.detailWidth; x++)
					{
						...
						float d1 = tile.primaryBiomeDistance;
						float d2 = tile.secondaryBiomeDistance;

						float exponentPower = 1.4f;
						...
						AddGrassForBiome(grassDict, detailIndex, detail, x, y, noGrassNoise, zoneGap, tile, detail2, map, 1,d1/sum);

						grassDict = BiomeManager.Instance.biomeMapping[secondaryBiome].indexToDetail;


						AddGrassForBiome(grassDict, detailIndex, detail, x, y, noGrassNoise, zoneGap, tile, detail2, map, 0.6f,d2/sum);
					}
				}
				terrainData.SetDetailLayer(0, 0, detailIndex, map);
			}
		}

		private int[,] AddGrassForBiome(Dictionary<int,BiomeGrassWrapper> grassDict, int detailIndex, GeneratorSettings detail,int x,int y, 
			Dictionary<string, GeneratorSettings> noGrassNoise, float zoneGap, Tile tile, GeneratorSettings detail2, int[,] map, float applyMod,
			float biomeDistance)
		{
			BiomeGrassWrapper layerDetail;
			if (grassDict.TryGetValue(detailIndex, out layerDetail) && layerDetail.spawnWeight >= detail.GetDataPoint(x, y)
							&& (layerDetail.noGrassChance < noGrassNoise[layerDetail.config.name].GetDataPoint(x, y) || layerDetail.config.onlyUseSteepness))
			{
				if (tile.steepnessValue > layerDetail.config.maxGrassSpawnAngle) { return map; }
				...
				for (int count = 1; count <= detailMaxDensity; count++)
				{
					if (detail2.GetDataPoint(x, y) <= count * zoneGap)
					{
						...
						map[x, y] = Mathf.RoundToInt(biomeDistance * applyMod * Mathf.Max(count, detailMinDensity));
						return map;
					}
				}
			}
